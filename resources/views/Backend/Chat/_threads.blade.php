        ...
        <div class="d-flex justify-content-between align-items-start gap-3">
            ...
            <div class="d-flex flex-column align-items-end">
                <div class="d-flex align-items-center gap-2">
                    <span class="chat-thread-time {{$isActive ? 'text-white opacity-75' : 'text-muted'}} fs-8">{{$thread->ultimoMessaggio?->created_at?->format('d/m H:i') ?? $thread->created_at?->format('d/m H:i')}}</span>
                    <button type="button"
                            class="btn btn-icon btn-sm {{$isActive ? 'btn-light' : 'btn-light-secondary'}} chat-thread-action-btn chat-thread-mute"
                            data-thread-id="{{$thread->id}}"
                    ...
                    <button type="button"
                            class="btn btn-icon btn-sm {{$isActive ? 'btn-light' : 'btn-light-secondary'}} chat-thread-action-btn chat-thread-close"
                            data-thread-id="{{$thread->id}}"
