        ...
        <div class="card-body">
            <div class="d-flex justify-content-center pb-4">
                <div class="d-flex align-items-center">
                    <div class="fw-bolder fs-4 text-center me-4">in collaborazione con:</div>
                    <img src="{{\App\Models\ServizioFinanziario::LOGHI_SERVIZI[$tipoServizio]['logo']}}"
                         class="{{\App\Models\ServizioFinanziario::LOGHI_SERVIZI[$tipoServizio]['size']}}">
                </div>
            </div>
            ...
            <form method="POST" action="{{action([$controller,'update'],$record->id??'')}}">
                ...
                <input type="hidden" name="tipo_servizio" value="{{old('tipo_servizio',$tipoServizio)}}">
                ...
                @include("Backend.ServizioFinanziario.Prodotti.$tipoServizio",['record'=>$recordProdotto])
                <div class="row">
                    <div class="col-md-4 offset-md-4 text-center">
                        <button class="btn btn-primary mt-3" type="submit"
                                id="submit">{{$vecchio?'Salva modifiche':'Crea '.\App\Models\ServizioFinanziario::NOME_SINGOLARE}}</button>
                    </div>
                    @if($vecchio)
