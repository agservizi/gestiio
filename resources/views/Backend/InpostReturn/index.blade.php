...
@section('toolbar')
    <div class="inpost-toolbar">
        <div class="inpost-toolbar-search" data-bs-toggle="tooltip" data-bs-placement="bottom" title="{{$testoCerca}}">
            ...
            <input type="text" id="filter_search" class="form-control form-control-solid inpost-toolbar-input" placeholder="Ricerca per destinatario"/>
        </div>
...
@section('content')
    @php
        ...
        $createdCount = $collection->filter(fn($r) => in_array(strtoupper((string)$r->status), ['CREATED','CONFIRMED','PENDING']))->count();
    ...
    <div class="inpost-index-page">
        <section class="inpost-hero-card mb-8">
            <div class="inpost-hero-copy">
                <div class="inpost-hero-kicker">Resi InPost</div>
                <h1 class="inpost-hero-title">Gestisci i resi dai tuoi clienti.</h1>
                <p class="inpost-hero-text">Crea un reso InPost: il cliente porta il pacco al punto di ritiro indicato. QR code e tracking inclusi.</p>
                <div class="inpost-hero-badges">
                    <span class="inpost-doc-badge">Punti di ritiro</span>
                    <span class="inpost-doc-badge">QR code</span>
                    <span class="inpost-doc-badge">Tracking</span>
                </div>
            </div>
            <div class="inpost-kpi-grid">
                <div class="inpost-kpi-card">
                    <span class="inpost-kpi-label">Totale resi</span>
                    <span class="inpost-kpi-value">{{$records->total()}}</span>
                </div>
                <div class="inpost-kpi-card">
                    <span class="inpost-kpi-label">Pagina</span>
        ...
        <div class="inpost-table-shell">
            <div class="inpost-table-shell-head">
                <div>
                    <h2 class="inpost-table-title">Elenco resi</h2>
                    <p class="inpost-table-text">Controlla stato e tracking di ogni reso.</p>
                </div>
            </div>
            <div class="card-body pt-0 pb-5 fs-6" id="tabella">
                @include('Backend.InpostReturn.tabella')
            </div>
        </div>
    </div>
@endsection
