    ...
    <div class="row g-5 g-xl-10 mb-5 mb-xl-10">
            ...
            <div class="card card-flush h-md-100">
                <div class="card-header border-0 pt-5">
                    <h3 class="card-title align-items-start flex-column">
                        <span class="card-label fw-bold fs-3 mb-1">Indicatori assistenza</span>
                    </h3>
                </div>
                <div class="card-body pt-2">
                    ...
                    <div class="d-flex justify-content-between mb-4">
                        <span class="text-muted">Richieste oggi</span>
                        <span class="fw-bolder">{{ number_format($kpiDashboard['richieste_assistenza_oggi']) }}</span>
                    </div>
                    <div class="d-flex justify-content-between mb-4">
                        <span class="text-muted">Clienti assistenza</span>
                        <span class="fw-bolder">{{ number_format($kpiDashboard['clienti_assistenza_totali']) }}</span>
                    </div>
                    <div class="d-flex justify-content-between">
                        <span class="text-muted">Ticket aperti</span>
                        <span class="fw-bolder">{{ number_format($kpiDashboard['ticket_aperti']) }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-xxl-4">
            <div class="card card-flush h-md-100">
                <div class="card-header border-0 pt-5">
                    <h3 class="card-title align-items-start flex-column">
                        <span class="card-label fw-bold fs-3 mb-1">Verifiche rapide</span>
                    </h3>
                </div>
                <div class="card-body pt-2">
                    <div class="d-flex justify-content-between mb-4">
                    ...
                    <div class="d-flex justify-content-between mb-6">
                        <span class="text-muted">Clienti senza contatti</span>
                        <span class="fw-bolder text-warning">{{ number_format($alertDashboard['clienti_senza_contatti']) }}</span>
                    </div>
                    <div class="d-flex flex-wrap gap-2">
                        <a class="btn btn-light-primary btn-sm me-2 mb-2" href="{{ action([\App\Http\Controllers\Backend\RichiestaAssistenzaController::class,'index']) }}">Richieste</a>
                        <a class="btn btn-light-primary btn-sm me-2 mb-2" href="{{ action([\App\Http\Controllers\Backend\ClienteAssistenzaController::class,'index']) }}">Clienti</a>
                        <a class="btn btn-light-primary btn-sm mb-2" href="{{ action([\App\Http\Controllers\Backend\TicketsController::class,'index']) }}">Ticket</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-xxl-4">
            <div class="card card-flush h-md-100">
                <div class="card-header border-0 pt-5">
                    <h3 class="card-title align-items-start flex-column">
                        <span class="card-label fw-bold fs-3 mb-1">Priorità operative</span>
                    </h3>
                </div>
                <div class="card-body pt-2">
                    @if($azioniRapide->isEmpty())
                        <div class="text-muted">Nessuna priorità disponibile.</div>
                    @else
                        @foreach($azioniRapide->take(4) as $azione)
                            <div class="d-flex justify-content-between align-items-start mb-4">
                                <div class="me-3">
                                    <div class="fw-bolder">#{{ $azione->id }} {{ $azione->cliente?->nominativo() ?? 'Cliente non associato' }}</div>
                                    <div class="text-muted fs-7">{{ $azione->prodotto->nome ?? 'Prodotto non associato' }}</div>
                                </div>
