<div class="card card-flush h-md-100">
    <div class="card-header border-0 pt-5 pb-2">
        <div class="card-title d-flex align-items-center gap-2">
            <span class="svg-icon svg-icon-2 text-primary">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path opacity="0.3" d="M4 5C4 3.89543 4.89543 3 6 3H18C19.1046 3 20 3.89543 20 5V19C20 20.1046 19.1046 21 18 21H6C4.89543 21 4 20.1046 4 19V5Z" fill="currentColor"/>
                    <path d="M8 8H16V10H8V8ZM8 12H16V14H8V12Z" fill="currentColor"/>
                </svg>
            </span>
            <div>
                <h3 class="card-title m-0">Caf / Patronato recenti</h3>
                <div class="text-muted fs-7">Ultime pratiche operative da supervisionare</div>
            </div>
        </div>
        <div class="card-toolbar">
            <a class="btn btn-sm btn-light-primary fw-bold" href="{{action([\App\Http\Controllers\Backend\CafPatronatoController::class,'index'])}}">Vedi tutti</a>
        </div>
    </div>

    <div class="card-body pt-0">
        <div class="table-responsive">
            <table class="table table-row-dashed align-middle gs-0 gy-3" id="tabella-elenco-supervisore-caf">
                <thead>
                <tr class="fw-bold text-muted fs-8 text-uppercase">
                    <th>Data</th>
                    <th>Tipo pratica</th>
                    <th>Esito</th>
                    <th>Nominativo</th>
                    <th class="text-center">Azioni</th>
                </tr>
                </thead>
                <tbody>
                @forelse($records as $record)
                    <tr id="tr_{{$record->id}}">
                        <td class="fw-semibold">{{$record->data?->format('d/m/Y')}}</td>
                        <td>{{$record->tipo?->nome}}</td>
                        <td>
                            <div class="d-flex align-items-center">
                                {!! $record->bulletEsitoFinale() !!}
                                <div class="me-1">{!! $record->esito?->labelStato() !!}</div>
                                @if($record->motivo_ko)
                                    <i class="fas fa-question-circle ms-1 fs-6" data-bs-toggle="tooltip" title="{{$record->motivo_ko}}"></i>
                                @endif
                            </div>
                        </td>
                        <td>{{$record->nominativo()}}</td>
                        <td class="text-end text-nowrap">
                            <div class="btn-group" role="group">
                                @if($record->allegati_per_cliente_count)
                                    <a href="{{action([\App\Http\Controllers\Backend\CafPatronatoController::class,'downloadAllegatoCliente'],$record->id)}}"
                                       class="btn btn-icon btn-sm btn-light btn-active-light-primary"
                                       data-bs-toggle="tooltip" data-bs-placement="top" title="Scarica documento">
                                        <span class="svg-icon svg-icon-muted svg-icon-1">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M17 11H7C6.4 11 6 10.6 6 10V9C6 8.4 6.4 8 7 8H17C17.6 8 18 8.4 18 9V10C18 10.6 17.6 11 17 11ZM22 5V4C22 3.4 21.6 3 21 3H3C2.4 3 2 3.4 2 4V5C2 5.6 2.4 6 3 6H21C21.6 6 22 5.6 22 5Z" fill="currentColor"/>
                                                <path opacity="0.3" d="M21 16H3C2.4 16 2 15.6 2 15V14C2 13.4 2.4 13 3 13H21C21.6 13 22 13.4 22 14V15C22 15.6 21.6 16 21 16ZM18 20V19C18 18.4 17.6 18 17 18H7C6.4 18 6 18.4 6 19V20C6 20.6 6.4 21 7 21H17C17.6 21 18 20.6 18 20Z" fill="currentColor"/>
                                            </svg>
                                        </span>
                                    </a>
                                @endif
                                @if($record->allegati_count)
                                    <a href="{{action([\App\Http\Controllers\Backend\ModalController::class,'show'],['modal-allegati-caf',$record->id])}}"
                                       class="btn btn-icon btn-sm btn-light btn-active-light-primary"
                                       data-target="kt_modal" data-toggle="modal-ajax"
                                       data-bs-toggle="tooltip" data-bs-placement="top" title="Scarica allegati">
                                        <span class="svg-icon svg-icon-muted svg-icon-1">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path opacity="0.3" d="M4.425 20.525C2.525 18.625 2.525 15.525 4.425 13.525L14.825 3.125C16.325 1.625 18.825 1.625 20.425 3.125C20.825 3.525 20.825 4.12502 20.425 4.52502C20.025 4.92502 19.425 4.92502 19.025 4.52502C18.225 3.72502 17.025 3.72502 16.225 4.52502L5.82499 14.925C4.62499 16.125 4.62499 17.925 5.82499 19.125C7.02499 20.325 8.82501 20.325 10.025 19.125L18.425 10.725C18.825 10.325 19.425 10.325 19.825 10.725C20.225 11.125 20.225 11.725 19.825 12.125L11.425 20.525C9.525 22.425 6.425 22.425 4.425 20.525Z" fill="currentColor"/>
                                                <path d="M9.32499 15.625C8.12499 14.425 8.12499 12.625 9.32499 11.425L14.225 6.52498C14.625 6.12498 15.225 6.12498 15.625 6.52498C16.025 6.92498 16.025 7.525 15.625 7.925L10.725 12.8249C10.325 13.2249 10.325 13.8249 10.725 14.2249C11.125 14.6249 11.725 14.6249 12.125 14.2249L19.125 7.22493C19.525 6.82493 19.725 6.425 19.725 5.925C19.725 5.325 19.525 4.825 19.125 4.425C18.725 4.025 18.725 3.42498 19.125 3.02498C19.525 2.62498 20.125 2.62498 20.525 3.02498C21.325 3.82498 21.725 4.825 21.725 5.925C21.725 6.925 21.325 7.82498 20.525 8.52498L13.525 15.525C12.325 16.725 10.525 16.725 9.32499 15.625Z" fill="currentColor"/>
                                            </svg>
                                        </span>
                                    </a>
                                @endif
                                <a class="btn btn-sm btn-light-primary" href="{{action([\App\Http\Controllers\Backend\CafPatronatoController::class,'edit'],$record->id)}}">Apri</a>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="text-muted">Nessuna pratica recente disponibile.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
        </div>
    </div>
</div>
