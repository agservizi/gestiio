<div class="table-responsive">
    <table class="table table-row-bordered align-middle gy-4" id="tabella-elenco">
        ...
        @forelse($records as $record)
            @php
                $canCancel = ! in_array($record->status->value, ['ANNULLATO', 'CONSEGNATO'], true);
            @endphp
            <tr>
                <td><a href="{{ action([$controller, 'show'], $record->id) }}" class="fw-bold">{{ $record->code }}</a></td>
                <td>
                    <div>{{ $record->recipient_name }}</div>
                    <div class="text-muted fs-7">{{ $record->recipient_email }}</div>
                </td>
                <td>{{ $record->sender_name ?: '—' }}</td>
                <td>{{ $record->carrier ?: '—' }}</td>
                <td>{{ $record->expected_pickup_date?->format('d/m/Y') }}</td>
                <td><span class="badge {{ $record->status->badgeClass() }}">{{ $record->status->label() }}</span></td>
                <td>{{ $record->source instanceof \App\Enums\LockerPackageSource ? $record->source->label() : ($record->source === 'online' ? 'Online' : 'Sportello') }}</td>
                            ...
                            @if($canCancel)
                                ...
                                <div class="menu-item px-3">
                                    <form method="post" action="{{ action([$controller, 'action'], $record->id) }}">
                                        ...
                                        <button type="button" class="menu-link px-3 w-100 border-0 bg-transparent text-start text-warning" onclick="return gestiioAsk(this, 'Annullare il pacco {{ $record->code }}?')">
        ...
        @empty
            <tr><td colspan="8" class="text-center text-muted py-10">Nessun pacco trovato.</td></tr>
        @endforelse
        </tbody>
    </table>
</div>
