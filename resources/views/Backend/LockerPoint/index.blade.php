...
@section('toolbar')
    <div class="d-flex flex-wrap align-items-center gap-2 py-1">
        @foreach(['oggi' => 'Oggi', 'prenotati' => 'Prenotati', 'giacenza' => 'In giacenza', 'consegnati' => 'Consegnati', 'annullati' => 'Annullati'] as $key => $label)
            <a href="{{ action([$controller, 'index'], ['view' => $key]) }}"
               class="btn btn-sm {{ ($view ?? 'oggi') === $key ? 'btn-primary' : 'btn-light' }}">{{ $label }}</a>
        @endforeach
        <div class="d-flex align-items-center position-relative ms-md-2" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Cerca codice o destinatario">
        ...
        <a class="btn btn-sm btn-primary fw-bold" href="{{ action([$controller, 'create']) }}">Nuovo {{ \App\Models\LockerPackage::NOME_SINGOLARE }}</a>
    </div>
@endsection
...
@section('content')
    ...
    <div class="card pt-4">
        <div class="card-body pt-0 pb-5 fs-6" id="tabella">
            @include('Backend.LockerPoint.tabella')
        </div>
    </div>
@endsection
