    ...
    <div class="d-flex align-items-center py-1">
        ...
        @isset($testoCerca)
            ...
            <span class="svg-icon svg-icon-3 position-absolute ms-3 mt-n1">
                ...
                <input type="text" id="filter_search" class="form-control form-control-sm form-control-solid  fw-bold fs-7 w-200px ps-9 bg-body btn-color-gray-700"
                       placeholder="Ricerca"/>
            </div>
        @endisset
        @isset($ordinamenti)
...
@endsection
@section('content')
    @php($visualizzaAgente=Auth::user()->hasPermissionTo('admin'))
            ...
            <div class="table-responsive">
                <table class="table table-row-bordered" id="tabella-elenco">
                    ...
                    <tr class="fw-bolder fs-6 text-gray-800">
                        ...
                        @endif
                        <th class="">Cliente</th>
                        <th class="">Contratto</th>
                        <th class="">Tipo</th>
                        <th class="text-end">Prezzo</th>
                        <th class="">Richiesta</th>
                        <th class="">Stato</th>
                        <th class="min-w-200px"></th>
                    </tr>
                    </thead>
                    <tbody id="tbody">
                    @include('Backend.VisuraCamerale.tabella')
                    </tbody>
                </table>
            </div>
...
@push('customScript')
    <script>
        ...
        $(function () {
            ...
            function handler() {
                $('.aggiorna-visura').off();
                $('.aggiorna-visura').click(function (e) {
                    e.preventDefault();
                    const $obj = $(this);
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.success) {
                                ...
                                $('#tr_' + data.oggettoReload).replaceWith(base64_decode(data.html));
                                handler();
