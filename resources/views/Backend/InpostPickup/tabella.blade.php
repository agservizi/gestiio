    ...
    <table class="table align-middle" id="tabella-elenco">
        ...
        @foreach($records as $record)
            <tr data-id="{{$record->id}}">
                ...
                <td>
                    <div class="fw-bold text-dark">{{$record->contact_name}}</div>
                    <div class="text-muted fs-7">{{$record->contact_phone}}</div>
                </td>
                <td>
                    <div class="fw-semibold">{{$record->indirizzoCompleto()}}</div>
                </td>
                <td>
                    <span class="fw-bold">{{$record->pickup_date?->format('d/m/Y') ?? '-'}}</span>
                </td>
                <td>
                    <span class="badge badge-light-primary">{{$record->parcel_count}}</span>
                </td>
