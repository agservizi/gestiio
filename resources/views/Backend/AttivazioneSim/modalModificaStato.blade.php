...
@section('content')
    <form id="form-aggiorna-stato" action="{{action([\App\Http\Controllers\Backend\AttivazioneSimController::class,'aggiornaStato'],$record->id)}}" method="POST">
        ...
        <div id="div_motivazione" class="mb-2">
            ...
            @if(!$record->motivo_ko)
                <div style="margin-top: -15px;">
                    @foreach(\App\Models\TabMotivoKo::perModal('attivazione')->get()->sortBy('nome') as $motivo)
...
@push('customScript')
    ...
    <script>
        $(function () {
            const esiti =@json(\App\Models\EsitoAttivazioneSim::select('id','esito_finale')->get()->keyBy('id'));
