...
class NotificaSendRejected extends Notification
{
    use Queueable;
    ...
    public function toMail($notifiable): MailMessage
    {
        $mail = (new MailMessage)
            ->subject('SEND: pratica rifiutata '.$this->request->request_number)
            ->line('La pratica SEND '.$this->request->request_number.' è stata rifiutata.');

        if ($this->request->rejection_reason) {
            $mail->line('Motivazione: '.$this->request->rejection_reason);
        }
