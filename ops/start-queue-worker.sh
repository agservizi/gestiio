...
su -s /bin/sh www-data -c \"cd /var/www/html && nohup php artisan queue:work --sleep=3 --tries=3 --max-time=3600 >> storage/logs/queue-worker.log 2>&1 &\"
sleep 1
pgrep -af \"artisan queue:work\" || { echo QUEUE_WORKER_START_FAILED; exit 1; }
